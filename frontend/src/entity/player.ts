...
export class Player extends Entity {
  ...
  setMouse(x, y) {
    if (this.speed <= 1 && this.bouncer <= 0) {
      this.mouseX = x;
      this.mouseY = y;
      this.rotate(x, y);
    }
  ...
  move() {
    ...
    let dist = Math.sqrt(
      Math.pow(myX - this.mouseX, 2) + Math.pow(myY - this.mouseY, 2)
    );
  ...
  rotate(x, y) {
    this.setRotation(
      Math.atan2(
        y - SCALE * this.getDisplayY(),
        x - SCALE * this.getDisplayX()
      ) +
        (3 * Math.PI) / 2
    );
  }
  bounce() {
    if (this.bouncer <= 0) {
      //do something to change direction
  ...
  bounceManager(delta) {
    if (this.bouncer > 0) {
      this.bouncer = this.bouncer - delta / 60;
    }
  }
  draw(delta: number) {
    this.speedManager(delta);
    this.bounceManager(delta);
  }
}
